...
export class AuthService {
  ...
  login(formData: any) {
    ...
    return this.http
      .post(environment.loginRestApi, formData) // send the login credentials to the REST API
      .pipe(
        map((res: any) => {
          // token is received as the response
          console.log(res);
          return res; // return response to the component
        }),
        catchError((err: any) => {
          //catching the error from the backend(REST API)
